...
/* 
    ...
    - Using a forEach loop Through the presidents and have it console.log( first name, last name XX years old when they dies.
        - /we only want the dead presidents to displayed.
*/

console.log(presidents.length);

const alivePres = presidents.filter((president) => president.passed === undefined);
console.table(alivePres);

const deadPres = presidents.filter((president) => president.passed !== undefined);
console.log(deadPres);

deadPres.forEach((presidents) => {
    const ageOfDeath = presidents.passed -presidents.year;
